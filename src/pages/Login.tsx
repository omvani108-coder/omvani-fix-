        ...
        <div className="bg-card rounded-xl p-8 shadow-sacred border border-border">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Link to="/forgot-password" className="text-xs text-saffron hover:underline font-sans">
            ...
            <Button type="submit" variant="hero" size="lg" className="w-full" disabled={loading || isLockedOut}>
              {loading ? t.auth.signingIn : isLockedOut ? "Locked — try again shortly" : t.nav.signIn}
            </Button>
          </form>

          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border" /></div>
            <div className="relative flex justify-center"><span className="bg-card px-3 text-xs text-muted-foreground font-sans">{t.auth.or}</span></div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              onClick={async () => {
                const { error } = await lovable.auth.signInWithOAuth("apple", { redirect_uri: window.location.origin + "/onboarding" });
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground font-sans">
            {t.auth.noAccount}{" "}
            <Link to="/signup" className="text-saffron hover:underline font-semibold">
              {t.auth.startTrial}
            </Link>
          </p>
        </div>
